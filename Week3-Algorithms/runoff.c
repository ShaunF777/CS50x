...
bool print_winner(void)
{
    ...
    for (int k = 0; k < candidate_count; k++) // loop through candidates
    {
        // printf ("%s: has %i votes. ", candidates[j].name, candidates[j].votes);
        if (candidates[k].votes >= ((int) winvotes))
        {
            printf("%s\n", candidates[k].name);
            return true;
        }
    ...
    for (int j = 0; j < candidate_count; j++) // loop through candidates
    {
        if (!candidates[j].eliminated) // only running candidates are included
        {
            if (candidates[j].votes == min)
            {
                ...
                // printf ("\nCandidate %s with %i votes has been eliminated.", candidates[j].name,
                // candidates[j].votes);
            }
        }
    }
